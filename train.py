...
TRAIN_CONFIG_FILE = DIRECTORY + '/rl_environments/configs/config_scheduling_25_5.toml'

def make_scheduling_env(config):
    from rl_environments.environment_scheduling_NSGAii import schedulingEnv
...
def main():
    ...
    config['environment']['kl_log_dir'] = model_logdir
    ...
    if config['environment']['nr_of_environments'] == 1:
        if 'scheduling' in TRAIN_CONFIG_FILE:
            train_env = DummyVectorEnv(
                [partial(make_scheduling_env, config) for _ in range(config['environment']['nr_of_environments'])])
