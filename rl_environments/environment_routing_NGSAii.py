...
def fit_gaussian(pop_objs, eps=1e-6):
    mu = pop_objs.mean(axis=0)
    cov = np.cov(pop_objs.T)
...
def kl_gaussian(mu0, cov0, mu1, cov1):
    ...
    term1 = np.trace(inv_cov1 @ cov0)
    term2 = (mu1 - mu0).T @ inv_cov1 @ (mu1 - mu0)
...
#                print("=== DEBUG ideal_points_path ===", ROUTING_DATA + filename)
#                print("=== DEBUG ideal_points_keys(sample)===", list(self.ideal_points.keys())[:30])
#                print("=== DEBUG ideal_points_has_1081 ===", "1081" in self.ideal_points)
    ...
    def _observe(self):
        obs = {'graph': GraphInstance(
            ...
            'additional_features': np.array([self.generation / self.max_generations,self.kl_norm],dtype=np.float32)
            }
    ...
    def reset(self):
        ...
        if not self.save_results:
            ...
            # === load reference points correctly ===
            size_to_file = {
        ...
        if not self.save_results:
            try:
                ip = self.ideal_points[str(self.problem_instance)]
            except KeyError:
                print(f"[WARNING] Ideal point for instance {self.problem_instance} not found. Using fallback.")
                # fallback 
                ip = [0.0, 0.0]

            self.ideal_hv = compute_hypervolume(
                [tuple(ip)],
                self.nr_objectives,
    ...
    def step(self, action):
        ...
        current_obj_array = np.array(
            [list(ind.fitness.values) for ind in self.population],
            dtype=np.float32
        )

        kl_value = 0.0
        if self.prev_obj_array is not None:
            mu0, cov0 = fit_gaussian(self.prev_obj_array)
            mu1, cov1 = fit_gaussian(current_obj_array)
            kl_value = kl_gaussian(mu0, cov0, mu1, cov1)

        self.prev_obj_array = current_obj_array
        self.kl_norm = kl_value / (kl_value + 1.0)
        ...
        if self.generation == self.max_generations:
            ...
            current_hv = compute_hypervolume(self.population, self.nr_objectives, self.reference_point)
            print(f"Final HV: {current_hv}, ref_point: {self.reference_point}")
