...
from scheduling.helper_functions import load_job_shop_env  #加载FSJP实例
...
def fit_gaussian(pop_objs, eps=1e-6):
    ...
    cov = np.atleast_2d(np.cov(pop_objs.T))
...
class schedulingEnv(gym.Env):
    ...
    def _observe(self):
        ...
        return obs
    """
    def _observe(self):#输入给图的特征（节点，边，连接）
        obs = {'graph': GraphInstance(
            nodes=np.array([fit for fit in normalize_fitnesses(self.population, self.bounds)]),
            edges=get_edges(self),
            edge_links=get_edge_links(self)),
            ...
            'additional_features': np.array(
                [self.generation / self.max_generations, self.kl_norm],
                dtype=np.float32
            )
            }
        return obs
    ...
    def step(self, action):
        ...
        return self._observe(), reward, self.done, None, {"kl_value": kl_value, "kl_norm": self.kl_norm}
